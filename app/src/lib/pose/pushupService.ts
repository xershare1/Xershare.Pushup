...
/** Exclusive elbow angle bounds (°) for “arms extended” at top of rep. Legacy was (160, 180). */
const UP_ELBOW_DEG = { min: 150, max: 180 } as const

/** Exclusive elbow angle bounds (°) for bottom of rep (with nose heuristic unchanged). Legacy was (70, 100). */
const DOWN_ELBOW_DEG = { min: 55, max: 90 } as const
...
export class PushupService {
  ...
  getFacingLegKneeAngleDeg(pose: Pose, direction: 'left' | 'right'): number | null {
    const hipIndex = direction === 'left' ? POSE_KEYPOINTS.LEFT_HIP : POSE_KEYPOINTS.RIGHT_HIP
    const kneeIndex = direction === 'left' ? POSE_KEYPOINTS.LEFT_KNEE : POSE_KEYPOINTS.RIGHT_KNEE
    const ankleIndex = direction === 'left' ? POSE_KEYPOINTS.LEFT_ANKLE : POSE_KEYPOINTS.RIGHT_ANKLE
  ...
  /**
   * True when the visible leg is extended enough for a full plank (not kneeling / break).
   * If the angle cannot be measured, returns false so rep phase resets conservatively.
   */
  isLegsExtendedPlank(pose: Pose, direction: 'left' | 'right'): boolean {
    const deg = this.getFacingLegKneeAngleDeg(pose, direction)
    if (deg === null) return false
    return deg >= MIN_KNEE_ANGLE_DEG
  }
}
