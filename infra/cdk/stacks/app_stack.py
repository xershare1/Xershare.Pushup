"""Product app: S3 + CloudFront + custom domain for dev or prod."""
...
class AppStack(Stack):
    """S3 + CloudFront for product app. Deploys from ../../app/dist."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        bucket = s3.Bucket(
            self,
            "Bucket",
            bucket_name=f"pushuppros-app-{environment}",
        ...
        dist_dir = Path(__file__).parent.parent.parent.parent / "app" / "dist"
        if not dist_dir.exists():
            Annotations.of(self).add_warning(
                f"app/dist not found. Run 'cd app && npm run build' before deploy."
            )
        else:
        ...
        record_name = domain.replace(".pushuppros.com", "")
        route53.ARecord(
            self,
            "Alias",
            zone=zone,
        ...
        CfnOutput(self, "SiteUrl", value=f"https://{domain}", description="Product app URL")
